...
export async function POST(request: NextRequest) {
  ...
  const martName = 'fact_contract_activity';
  ...
  try {
    ...
    const sqlPath = path.join(process.cwd(), 'analytics/sql/marts/fact_contract_activity.sql');
    ...
    // Revalidate analytics contract activity API and page
    revalidatePath('/api/analytics/contract-activity');
    revalidatePath('/contract-activity');
