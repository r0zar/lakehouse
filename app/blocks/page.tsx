...
function ExpandedRow({ block }: ExpandedRowProps) {
  const blockDetails = {
    identifiers: {
      block_hash: block.block_hash,
      block_index: block.block_index,
      stacks_block_hash: block.stacks_block_hash,
    },
    bitcoin_anchor: {
      hash: block.bitcoin_anchor_hash,
      index: block.bitcoin_anchor_index,
    },
    metadata: {
      block_time: block.block_time,
      transaction_count: block.transaction_count,
      is_streaming_blocks: block.is_streaming_blocks,
    },
    webhook_info: {
      event_id: block.event_id,
      webhook_path: block.webhook_path,
      chainhook_uuid: block.chainhook_uuid,
      ...
      <td colSpan={6} className="p-0">
        ...
        >
          <Editor
            ...
            value={JSON.stringify(blockDetails, null, 2)}
            theme="vs-dark"
            options={{
              readOnly: true,
...
export default function BlocksPage() {
  ...
  const fetchBlocks = async (page: number = 1) => {
    try {
      ...
      const response = await fetch(`/api/blocks?page=${page}&limit=50`);
  ...
  useEffect(() => {
    ...
    // Auto-refresh every 30 seconds, but only for page 1 to show latest blocks
  ...
  const toggleExpanded = (blockHash: string) => {
    setExpandedRow(expandedRow === blockHash ? null : blockHash);
  };
  ...
  const goToPage = (page: number) => {
    if (page >= 1 && (!blocksData || page <= Math.ceil(blocksData.totalCount / blocksData.limit))) {
      fetchBlocks(page);
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Failed to Load Blocks</h3>
        ...
        <header className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-300">
                Processed blockchain blocks with transaction counts and metadata
              </p>
            </div>
            <a
              href="/"
          ...
          {blocksData && (
            <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
              <div>
                Showing {((blocksData.page - 1) * blocksData.limit) + 1}-{Math.min(blocksData.page * blocksData.limit, blocksData.totalCount)} of {blocksData.totalCount.toLocaleString()} blocks
              </div>
              <div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {blocksData?.blocks.map((block, index) => (
                  <React.Fragment key={block.block_hash}>
                    <motion.tr
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <motion.button
                          onClick={() => toggleExpanded(block.block_hash)}
                        ...
                        >
                          <motion.svg
                            ...
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            animate={{ rotate: expandedRow === block.block_hash ? 180 : 0 }}
                            transition={{ duration: 0.3 }}
                          >
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                          </motion.svg>
                          {expandedRow === block.block_hash ? 'Collapse' : 'Expand'}
                        </motion.button>
                      </td>
                    </motion.tr>
                    <AnimatePresence>
                      {expandedRow === block.block_hash && <ExpandedRow block={block} />}
                    </AnimatePresence>
                  </React.Fragment>
                ))}
              </tbody>
          ...
          <div className="mt-6 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="px-3 py-2 text-sm text-gray-700 dark:text-gray-300">
                Page {currentPage} of {Math.ceil(blocksData.totalCount / blocksData.limit)}
              </span>
              <button
                onClick={() => goToPage(currentPage + 1)}
                disabled={!blocksData.hasMore || loading}
                className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Next
              </button>
